...
class AnalyticInverseKinematics:
    def __init__(self, base_frame="base_link", tool_frame="wx200/gripper_link"):
        # robot parmas
        #rclpy.init()
        self.node = rclpy.create_node('ik_solver')

        self.buffer=Buffer()
        self.executor = rclpy.executors.SingleThreadedExecutor()
        self.executor.add_node(self.node)
        listener = TransformListener(buffer = self.buffer, node = self.node, spin_thread=True)
        #self.executor.spin_once()



        dur = Duration(seconds=5.0)
        time_stamp = rclpy.time.Time()

        trans = self.buffer.lookup_transform(
             'wx200/base_link', "wx200/shoulder_link", time_stamp,dur
        )
        #print(trans)
        self.base_offset = np.array([trans.transform.translation.x, trans.transform.translation.y, trans.transform.translation.z])
        print(self.base_offset)

        trans = self.buffer.lookup_transform(
              "wx200/shoulder_link","wx200/upper_arm_link",time_stamp, dur
        )
        self.l1 = trans.transform.translation.z

        trans = self.buffer.lookup_transform(
             "wx200/upper_arm_link", "wx200/forearm_link", time_stamp, dur
        )
        self.l2 = math.sqrt(trans.transform.translation.x ** 2 + trans.transform.translation.z ** 2)
        self.angle2_bias = math.atan2(trans.transform.translation.x, trans.transform.translation.z)

        trans = self.buffer.lookup_transform(
           "wx200/forearm_link", "wx200/wrist_link", time_stamp, dur
        )
        self.l3 = trans.transform.translation.x

        trans = self.buffer.lookup_transform( "wx200/wrist_link",tool_frame, time_stamp, dur)
        self.l4 = trans.transform.translation.x
    ...
    def ik(self, position, alpha=0.0, cur_arm_config=4 * [0]):
        ...
        if not self.joint_limits[0][0] < theta[0] < self.joint_limits[0][1]:
            print('out of joint limits range')
            return None
        ...
        # 2) which ever satisfy constraints
            ...
            print('no theta found that satisfies constraints')
            return None
